...
BRIDGE_ASSET_FROM_BRIDGE_SPINE = {
    ...
    # Most Recent Appraisal Order Date is a carry-forward/validated-update field.
    # Do not map it directly from current SF Bridge for every row.
...
BRIDGE_ASSET_FROM_VALUATION = {
    ...
    "Origination As-Is Value": "Origination As-Is Value",
    "Origination ARV": "Origination After Repair Value",
    # IMPORTANT: Do not generically adopt updated/current valuation fields from
    # SF Valuation. The completed report preserves prior updated valuation values
    # unless a separate validated updated appraisal rule says to refresh them.
}
...
def _recompute_bridge_asset_funded_amount(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if {"Renovation Holdback", "Renovation Holdback Funded", "Renovation Holdback Remaining"}.issubset(out.columns):
        ...
        existing_remaining = pd.to_numeric(out["Renovation Holdback Remaining"], errors="coerce")
        # Source value wins. Only calculate remaining when the source remaining
        # field is blank. Recalculating over a populated source value created
        # Bridge Asset and Bridge Loan renovation rollup mismatches.
        out["Renovation Holdback Remaining"] = existing_remaining.where(
            existing_remaining.notna(),
            calc_remaining.where(has_calc),
        )
    return downcast_numeric_frame(out)
...
def build_bridge_asset(
    ...
    # Use the final out rows, not the original sf_spine Series, so these values
    # stay aligned by Asset ID after valuation/AM/NPL/carry-forward merges.
    prop_npd = pd.to_datetime(out.get("Property Next Payment Date", pd.Series([pd.NaT] * len(out), index=out.index)), errors="coerce")
    opp_npd = pd.to_datetime(out.get("Opportunity Next Payment Date", pd.Series([pd.NaT] * len(out), index=out.index)), errors="coerce")
    ...
    if "bridge_asset_manual" in prev_maps:
        ...
        keep_cols = ["_asset_key"] + [c for c in [
            ...
            "Construction Mgr.", "CM Assigned Date", "Servicer", "Servicer Status",
            "Remedy Plan", "Delinquency Notes", "Maturity Status", "Title Company",
            # Updated valuation fields are prior-first. Do not backfill these from
            # origination/generic valuation fields; preserve the completed report value.
            "Most Recent Appraisal Order Date", "Updated Valuation Date", "Updated As-Is Value", "Updated ARV",
            "Deal Intro Sub-Source", "Referral Source Account", "Referral Source Contact",
        ] if c in man.columns]
...
def build_bridge_loan(
        ...
        active_roll = pd.DataFrame(
            {
                ...
                "Next Payment Date_active": g["Next Payment Date"].apply(_min_dt) if "Next Payment Date" in ba.columns else pd.NaT,
    ...
    # Completed Bridge Loan UPB is the sum of finalized Bridge Asset UPB.
    # Loan-level/servicer/SF values are fallback only when the asset rollup is blank.
    final_upb = valid_asset_upb.where(valid_asset_upb.notna(), valid_loan_upb)
    ...
    asset_npd = pd.to_datetime(out.get("Next Payment Date_active", pd.Series([pd.NaT] * len(out), index=out.index)), errors="coerce")
    cur_npd = pd.to_datetime(out.get("Next Payment Date", pd.Series([pd.NaT] * len(out), index=out.index)), errors="coerce")
    serv_npd = pd.to_datetime(out.get("_serv_next_payment_date", pd.Series([pd.NaT] * len(out), index=out.index)), errors="coerce")
    out["Next Payment Date"] = asset_npd.where(asset_npd.notna(), cur_npd)
    out["Next Payment Date"] = out["Next Payment Date"].where(out["Next Payment Date"].notna(), serv_npd)
...
# -----------------------------------------------------------------------------
# File-only Salesforce export helpers
# -----------------------------------------------------------------------------
# These helpers let the app build from manually exported Salesforce report files
# instead of Salesforce OAuth / Bulk API. They standardize the exported report
# headers so the existing build_* functions can be reused unchanged.


def _uploaded_name(upload) -> str:
    if upload is None:
        return ""
    if isinstance(upload, (str, Path)):
        return Path(upload).name
    return getattr(upload, "name", "uploaded file")


def _uploaded_bytes(upload) -> bytes:
    if upload is None:
        return b""
    ...
    if hasattr(upload, "read"):
        ...
        try:
            ...
            pass
        return data
    raise TypeError(f"Unsupported upload object: {type(upload)!r}")


def _read_manual_excel_export(upload, preferred_contains: Optional[str] = "report") -> pd.DataFrame:
    """Read a manually exported Salesforce xlsx/csv report.

    Salesforce report exports often include a blank Sheet1 plus a report#### sheet.
    This chooses the first sheet with a real header row, preferring sheet names
    containing 'report'. Pandas deduplicates duplicate headers as .1, .2, etc.
    """
    raw = _uploaded_bytes(upload)
    name = _uploaded_name(upload)
    if not raw:
        return pd.DataFrame()
    lower = name.lower()
    ...
    if preferred_contains:
        pref = [s for s in candidates if preferred_contains.lower() in str(s).lower()]
        candidates = pref + [s for s in candidates if s not in pref]

    best_sheet = candidates[0]
    best_count = -1
    for sheet in candidates:
        try:
            sample = pd.read_excel(BytesIO(raw), sheet_name=sheet, header=None, nrows=1, dtype=object)
            count = int(sample.iloc[0].notna().sum()) if not sample.empty else 0
            if count > best_count:
                best_sheet, best_count = sheet, count
            if preferred_contains and preferred_contains.lower() in str(sheet).lower() and count >= 2:
                best_sheet = sheet
                break
        except Exception:
            continue
    df = pd.read_excel(BytesIO(raw), sheet_name=best_sheet, dtype=object)
    df = df.dropna(how="all")
    # Normalize stray whitespace in headers but keep pandas duplicate suffixes.
    df.columns = [clean_text(c) for c in df.columns]
    return downcast_numeric_frame(df)


def _first_matching_col(df: pd.DataFrame, base_name: str) -> Optional[str]:
    if df is None or df.empty:
        return None
    base = clean_text(base_name)
    for c in df.columns:
        if clean_text(c) == base:
            return c
    for c in df.columns:
        if clean_text(c).startswith(base + "."):
            return c
    return None


def _duplicate_like_cols(df: pd.DataFrame, base_name: str) -> List[str]:
    base = clean_text(base_name)
    out = []
    for c in df.columns:
        cc = clean_text(c)
        if cc == base or cc.startswith(base + "."):
            out.append(c)
    return out


def _copy_col_if_present(df: pd.DataFrame, src: str, dst: str, overwrite: bool = False) -> None:
    if src in df.columns and (overwrite or dst not in df.columns):
        df[dst] = df[src]


def _standardize_sf_bridge_export(upload) -> pd.DataFrame:
    df = _read_manual_excel_export(upload)
    if df.empty:
        return df

    # SF Bridge export has two columns both named "Next Payment Date".
    # In the uploaded report: first is property-level; second is opportunity/deal-level.
    npd_cols = _duplicate_like_cols(df, "Next Payment Date")
    if len(npd_cols) >= 1:
        df["Property Next Payment Date"] = df[npd_cols[0]]
    if len(npd_cols) >= 2:
        df["Opportunity Next Payment Date"] = df[npd_cols[1]]
    elif len(npd_cols) >= 1:
        df["Opportunity Next Payment Date"] = df[npd_cols[0]]

    # API build expects the Opportunity-level suspense label.
    if "Suspense Balance" in df.columns and "Salesforce Suspense Balance" not in df.columns:
        df["Salesforce Suspense Balance"] = df["Suspense Balance"]

    # Manual report naming differences.
    _copy_col_if_present(df, "Appraisal Order Date", "Most Recent Appraisal Order Date")
    _copy_col_if_present(df, "Current Appraisal Date", "Current Appraisal Date")
    _copy_col_if_present(df, "Current Appraised As-Is Value", "Current Appraised As-Is Value")
    if "Current Appraised After Repair Value" not in df.columns:
        for cand in ["After Repair Value", "Current Appraised After Repair Value", "Origination After Repair Value"]:
            if cand in df.columns:
                df["Current Appraised After Repair Value"] = df[cand]
                break

    # Ensure key fields are text-cleaned the same way API results are.
    for c in ["Servicer Loan Number", "Servicer Commitment Id", "Deal Loan Number", "Asset ID", "Property ID"]:
        if c in df.columns:
            df[c] = df[c].astype("string").str.strip().replace({"": pd.NA})
    if {"Servicer Loan Number", "Servicer Commitment Id"}.issubset(df.columns):
        df["Servicer Loan Number"] = coalesce_keep_nonblank(df["Servicer Loan Number"], df["Servicer Commitment Id"])
...
def _standardize_sf_term_export(upload) -> pd.DataFrame:
    df = _read_manual_excel_export(upload)
    if df.empty:
        return df
    for c in ["Deal Loan Number", "Servicer Commitment Id", "External Deal ID", "Yardi ID"]:
        if c in df.columns:
            df[c] = df[c].astype("string").str.strip().replace({"": pd.NA})
    # Reuse alternate external deal ID as a low-priority term servicer key if present.
    if "External Deal ID" in df.columns and "Term Servicer Key 1" not in df.columns:
        df["Term Servicer Key 1"] = df["External Deal ID"]
    return downcast_numeric_frame(df)


def _standardize_sf_valuation_export(upload) -> pd.DataFrame:
    df = _read_manual_excel_export(upload)
    if df.empty:
        return df
    _copy_col_if_present(df, "Order Date", "Most Recent Appraisal Order Date")
    for c in ["Deal Loan Number", "Asset ID", "Appraisal: Appraisal ID"]:
        if c in df.columns:
            df[c] = df[c].astype("string").str.strip().replace({"": pd.NA})
    return downcast_numeric_frame(df)


def _standardize_sf_am_export(upload) -> pd.DataFrame:
    df = _read_manual_excel_export(upload)
    if df.empty:
        return df
    # The manual report already uses these names in the uploaded file:
    # Deal Loan Number, Team Role, Team Member Name, Date Assigned.
    for c in ["Deal Loan Number", "Team Role", "Team Member Name"]:
        if c in df.columns:
            df[c] = df[c].astype("string").str.strip().replace({"": pd.NA})
    return downcast_numeric_frame(df)


def _standardize_optional_term_asset_export(upload) -> pd.DataFrame:
    if upload is None:
        return pd.DataFrame()
    df = _read_manual_excel_export(upload)
    if df.empty:
        return df
    # Try to normalize common column aliases from a manual Property/Term Asset report.
    alias_map = {
        "Deal Number": "Deal Loan Number",
        "# Units": "# of Units",
        "Property ALA": "ALA",
    ...
    if "Value Date" not in df.columns:
        df["Value Date"] = pd.NaT
    if "As-Is Value" not in df.columns:
        df["As-Is Value"] = np.nan
    for c in ["Deal Loan Number", "Asset ID", "Property ID"]:
        if c in df.columns:
            df[c] = df[c].astype("string").str.strip().replace({"": pd.NA})
    return downcast_numeric_frame(df)


def _build_dnl_from_manual_exports(sf_bridge: pd.DataFrame, sf_term: pd.DataFrame) -> pd.DataFrame:
    frames = []
    for df in [sf_bridge, sf_term]:
        if df is not None and not df.empty and {"Deal Loan Number", "Do Not Lend"}.issubset(df.columns):
            frames.append(df[["Deal Loan Number", "Do Not Lend"]].copy())
    if not frames:
        return pd.DataFrame(columns=["Deal Loan Number", "Do Not Lend"])
    out = pd.concat(frames, ignore_index=True).dropna(subset=["Deal Loan Number"])
    out["_deal_key"] = norm_id_series(out["Deal Loan Number"])
    out = out.dropna(subset=["_deal_key"]).drop_duplicates("_deal_key", keep="last").drop(columns=["_deal_key"])
    return downcast_numeric_frame(out)


def _build_bridge_property_rollup_from_export(sf_bridge: pd.DataFrame) -> pd.DataFrame:
    if sf_bridge is None or sf_bridge.empty or "Deal Loan Number" not in sf_bridge.columns:
        return pd.DataFrame(columns=["_deal_key", "Number of Assets", "# of Units", "State(s)", "Last Funding Date", "Active Asset Count", "Active Asset UPB"])
    df = sf_bridge.copy()
    df["_deal_key"] = norm_id_series(df.get("Deal Loan Number", pd.Series([None] * len(df), index=df.index)))
    df["_asset_key"] = norm_id_series(df.get("Asset ID", pd.Series([None] * len(df), index=df.index)))
    status = df.get("Status", pd.Series([pd.NA] * len(df), index=df.index)).astype("string").str.strip()
    is_active_asset = status.isin(BRIDGE_ACTIVE_PROPERTY_STATUSES)
    asset_upb = pd.to_numeric(df.get("Current UPB", pd.Series([np.nan] * len(df), index=df.index)), errors="coerce").fillna(0)
    df["_active_asset_upb"] = asset_upb.where(is_active_asset, 0.0)
    df["_is_active_asset"] = is_active_asset.astype("int8")
    df = df.dropna(subset=["_deal_key"])
    if df.empty:
        return pd.DataFrame(columns=["_deal_key", "Number of Assets", "# of Units", "State(s)", "Last Funding Date", "Active Asset Count", "Active Asset UPB"])
    g = df.groupby("_deal_key", dropna=True)
    out = pd.DataFrame(
        {
            ...
            "# of Units": pd.to_numeric(g["# of Units"].sum(min_count=1), errors="coerce") if "# of Units" in df.columns else np.nan,
            "State(s)": g["State"].apply(lambda s: ", ".join(sorted({clean_text(x) for x in s if clean_text(x)}))) if "State" in df.columns else pd.Series(dtype="string"),
            "Last Funding Date": g["Last Funding Date"].apply(lambda s: pd.to_datetime(s, errors="coerce").dropna().max() if len(pd.to_datetime(s, errors="coerce").dropna()) else pd.NaT) if "Last Funding Date" in df.columns else pd.NaT,
            "Active Asset Count": g["_is_active_asset"].sum(),
            "Active Asset UPB": pd.to_numeric(g["_active_asset_upb"].sum(min_count=1), errors="coerce"),
        }
    ).reset_index()
...
def _append_missing_prior_term_loan_rows(term_loan_df: pd.DataFrame, prev_maps: dict, sf_term: pd.DataFrame) -> Tuple[pd.DataFrame, List[str]]:
    """File-only safety net: preserve prior report Term Loan rows not emitted by current filters.

    Manual SF Term exports often have blank/stale Current Servicer UPB for loans that are still
    present in the completed report. The real report carries those rows forward and then refreshes
    where source files have values. This appends prior rows missing from the generated output,
    limited to deals still present in the current SF Term export when that export is available.
    """
    diagnostics: List[str] = []
    if not prev_maps or "term_loan_manual" not in prev_maps:
        return term_loan_df, diagnostics
    prev = prev_maps["term_loan_manual"].copy()
    if prev.empty or "_deal_key" not in prev.columns:
        return term_loan_df, diagnostics
    out = term_loan_df.copy() if term_loan_df is not None else pd.DataFrame()
    if "_deal_key" not in out.columns:
        if "Deal Number" in out.columns:
            out["_deal_key"] = norm_id_series(out["Deal Number"])
        else:
            out["_deal_key"] = pd.Series(dtype="object")
    existing = set(out.get("_deal_key", pd.Series(dtype="object")).dropna().astype(str).tolist())
    current_keys = set()
    if sf_term is not None and not sf_term.empty and "Deal Loan Number" in sf_term.columns:
        current_keys = set(norm_id_series(sf_term["Deal Loan Number"]).dropna().astype(str).tolist())
    missing = prev[~prev["_deal_key"].astype(str).isin(existing)].copy()
    if current_keys:
        missing = missing[missing["_deal_key"].astype(str).isin(current_keys)].copy()
    if missing.empty:
    ...
    out = pd.concat([out, missing], ignore_index=True, copy=False)
    diagnostics.append(f"Term Loan carry-forward rows appended from prior workbook: {len(missing):,}")
    return downcast_numeric_frame(out), diagnostics

def _build_bridge_loan_wide_from_bridge_export(sf_bridge: pd.DataFrame) -> pd.DataFrame:
    if sf_bridge is None or sf_bridge.empty or "Deal Loan Number" not in sf_bridge.columns:
        return pd.DataFrame()
    src = sf_bridge.copy()
    ...
    mapping = {
        ...
        "Primary Contact": "Primary Contact: Full Name",
        "Origination Date": "Close Date",
        "Original Maturity Date": "Original Loan Maturity Date",
        "Current Maturity Date": "Current Loan Maturity date",
        "Loan Commitment": "Loan Commitment",
        "Remaining Commitment": "Total Remaining Commitment Amount",
        "SF Suspense Balance": "Salesforce Suspense Balance",
        "SF Current UPB": "Current Servicer UPB",
        "Loan Stage": "Stage",
        "Product Type": "Product Type",
        "Product Sub Type": "Product Sub-Type",
        "Transaction Type": "Transaction Type",
        "Project Strategy": "Project Strategy",
        "CV Originator": "CAF Originator: Full Name",
        "Deal Intro Sub-Source": "Deal Intro Sub-Source",
        "Referral Source Account": "Referral Source Account: Account Name",
        "Referral Source Contact": "Referral Source Contact: Full Name",
        "AM Commentary": "Comments AM",
    ...
    for dst, src_col in mapping.items():
        out[dst] = src[src_col] if src_col in src.columns else pd.NA
    out["Next Payment Date"] = src.get("Opportunity Next Payment Date", src.get("Property Next Payment Date", pd.Series([pd.NaT] * len(src), index=src.index)))
    out["Servicer ID"] = coalesce_keep_nonblank(src.get("Servicer Loan Number", pd.Series([pd.NA] * len(src), index=src.index)), src.get("Servicer Commitment Id", pd.Series([pd.NA] * len(src), index=src.index)))
    out["Deal Number"] = out.get("Deal Number", pd.Series([pd.NA] * len(out), index=out.index)).astype("string").str.strip().replace({"": pd.NA})
    out["_deal_key"] = norm_id_series(out["Deal Number"])
    out["_nonnull_score"] = 0
    for c in ["Deal Name", "Loan Buyer", "Financing", "Servicer ID", "Loan Commitment", "SF Current UPB", "Next Payment Date"]:
        if c in out.columns:
            out["_nonnull_score"] = out["_nonnull_score"] + (~blankish_mask(out[c])).astype("int8")
    out = out.dropna(subset=["_deal_key"]).sort_values(["_deal_key", "_nonnull_score"]).drop_duplicates("_deal_key", keep="last")
    out = out.drop(columns=["_deal_key", "_nonnull_score"], errors="ignore")
    return downcast_numeric_frame(out)
...
st.set_page_config(page_title="Active Loans Builder - File Only", layout="wide")
st.title("Active Loans Report Builder — File-Only SF Exports")
st.subheader(hey())
...
st.markdown(
    f"""
Welcome! This file-only version builds the **Active Loans** workbook without Salesforce OAuth/API.

### What you’ll upload
1) Prior/completed **Active Loans** workbook for template + carry-forward
2) Manually exported Salesforce reports: **SF Bridge**, **SF Term**, **SF Valuation**, **SF AM Assignments**
3) External servicer files: Midland, FCI, Berkadia, Statebridge, CHL/Shellpoint payment CSV, etc.
4) Optional manual Term Asset / Property export if you have one. If omitted, Term Asset uses the prior completed workbook as the spine.

### UPB header
Preview default: **{run_dt.isoformat()}** → **{upb_col}**. During the build, the final UPB header is reset to the dominant uploaded servicer tape filename date.

**Important:** this version intentionally treats the prior/completed workbook as part of the process. The real Active Loan Report is not a clean rebuild from current files alone.
"""
)
...
try:
    ...
    st.success(f"✅ Repo template available: {_tmpl_path_used}")
except Exception as e:
    st.info("Repo template not found. Upload the prior/completed Active Loans workbook to use as the build base.")
    st.caption(str(e))

if POSTBUILD_AUDIT_AVAILABLE:
    st.caption("Post-build QA audit helper loaded. Completed workbooks can include QA Summary and QA Exceptions tabs.")
else:
    st.warning(f"Post-build QA audit helper was not loaded: {POSTBUILD_AUDIT_IMPORT_ERROR}")

st.markdown("### Step 1: Upload prior/completed workbook")
prev_upload = st.file_uploader(
    "Upload prior/completed Active Loans report (.xlsx) for template + carry-forward",
    type=["xlsx"],
    key="prev_active_loan_file_only",
)

st.markdown("### Step 2: Upload manually exported Salesforce reports")
sf_col1, sf_col2 = st.columns(2)
with sf_col1:
    sf_bridge_upload = st.file_uploader("SF Bridge export (.xlsx)", type=["xlsx"], key="sf_bridge_file_only")
    sf_term_upload = st.file_uploader("SF Term export (.xlsx)", type=["xlsx"], key="sf_term_file_only")
    sf_val_upload = st.file_uploader("SF Valuation export (.xlsx)", type=["xlsx"], key="sf_val_file_only")
with sf_col2:
    sf_am_upload = st.file_uploader("SF AM Assignments export (.xlsx)", type=["xlsx"], key="sf_am_file_only")
    sf_term_asset_upload = st.file_uploader(
        "Optional SF Term Asset / Property export (.xlsx)",
        type=["xlsx"],
        key="sf_term_asset_file_only",
        help="Optional. If omitted, Term Asset carries forward from the prior/completed workbook and allocates UPB from Term Loan.",
    )

st.markdown("### Step 3: Upload external servicer files")
servicer_uploads = st.file_uploader(
    "Upload current EXTERNAL servicer files (csv/xlsx)",
    type=["csv", "xlsx"],
    accept_multiple_files=True,
    key="servicer_files_file_only",
)

st.markdown("### Step 4: Build options")
skip_servicer_files = st.checkbox(
    "Skip servicer files",
    value=False,
    help="Not recommended for production. Leaves servicer-driven columns blank or file/SF fallback where available.",
)
...
if st.button("Clear cached parsing", type="secondary"):
    st.cache_data.clear()
    st.success("Cleared Streamlit data cache.")

build_btn = st.button("Build file-only report", type="primary")
...
if build_btn:
    ...
    if prev_upload is None and not _repo_template_available:
        required_missing.append("prior/completed Active Loans workbook or repo template")
    if need_bridge and sf_bridge_upload is None:
        required_missing.append("SF Bridge export")
    if need_term and sf_term_upload is None:
        required_missing.append("SF Term export")
    if (need_bridge or need_term) and sf_am_upload is None:
        required_missing.append("SF AM Assignments export")
    if need_bridge and sf_val_upload is None:
        required_missing.append("SF Valuation export")
    if not skip_servicer_files and not servicer_uploads:
        required_missing.append("external servicer files or check Skip servicer files")

    if required_missing:
        st.error("Missing required input(s): " + "; ".join(required_missing))
    else:
        wb = None
        serv_join = pd.DataFrame()
        serv_preview = pd.DataFrame()
        try:
            status = st.status("Preparing file-only build...", expanded=True)
            diagnostics: List[str] = []
            prev_maps: dict = {}
            ...
            if prev_upload:
                ...
                diagnostics.append("No prior/completed Active Loans workbook uploaded: using repo template only; carry-forward fields will be limited.")
            ...
            if skip_servicer_files:
                ...
                st.caption("Servicer files were skipped.")
            ...
            upb_col = make_upb_header(run_dt)
            diagnostics.append(f"File-only build; report as-of date used for UPB/DQ labels: {run_dt.isoformat()} ({upb_col})")

            status.update(label="Loading Excel template...")
            if prev_upload is None and not _repo_template_available:
                raise FileNotFoundError("No repo template found and no prior/completed workbook uploaded.")
            ...
            status.update(label="Reading manually exported Salesforce files...")
            sf_bridge = _standardize_sf_bridge_export(sf_bridge_upload) if sf_bridge_upload is not None else pd.DataFrame()
            sf_term = _standardize_sf_term_export(sf_term_upload) if sf_term_upload is not None else pd.DataFrame()
            sf_val = _standardize_sf_valuation_export(sf_val_upload) if sf_val_upload is not None else pd.DataFrame()
            sf_am = _standardize_sf_am_export(sf_am_upload) if sf_am_upload is not None else pd.DataFrame()
            sf_active_rm = pd.DataFrame()
            term_asset_file_source = _standardize_optional_term_asset_export(sf_term_asset_upload)
            bridge_dnl = _build_dnl_from_manual_exports(sf_bridge, sf_term)

            diagnostics.append(f"SF Bridge rows: {len(sf_bridge):,}")
            diagnostics.append(f"SF Term rows: {len(sf_term):,}")
            diagnostics.append(f"SF Valuation rows: {len(sf_val):,}")
            diagnostics.append(f"SF AM Assignment rows: {len(sf_am):,}")
            if not term_asset_file_source.empty:
                diagnostics.append(f"Optional SF Term Asset rows: {len(term_asset_file_source):,}")
            elif need_term_asset:
                diagnostics.append("No optional SF Term Asset export uploaded: Term Asset will use prior/completed workbook as spine, then allocate UPB from Term Loan.")

            if need_bridge:
                status.update(label="Preparing Bridge file-only source data...")
                bridge_spine = sf_bridge.copy()
                bridge_loan_wide = _build_bridge_loan_wide_from_bridge_export(sf_bridge)
                bridge_property_rollup = _build_bridge_property_rollup_from_export(sf_bridge)
                bridge_asset_ids = _bridge_asset_ids_from_spine(bridge_spine)
                bridge_val = sf_val.copy()
                ...
                del bridge_spine, bridge_loan_wide, bridge_property_rollup, bridge_asset_ids, bridge_val, bridge_asset_df
                gc.collect()

            if need_term:
                status.update(label="Preparing Term file-only source data...")
                term_wide = sf_term.copy()
                ...
                term_asset_filter_deals = candidate_term_deals
                diagnostics.append(f"Term source deal universe: {len(term_asset_filter_deals):,} deals")
                ...
                term_loan_df, term_loan_append_diags = _append_missing_prior_term_loan_rows(term_loan_df, prev_maps, sf_term)
                diagnostics.extend(term_loan_append_diags)
                ...
                if need_term_asset:
                    ...
                    term_asset_source = term_asset_file_source.copy()
                    ...
                    del term_asset_source, term_asset_df
            ...
            if run_postbuild_qa and ENFORCE_ZERO_FILLABLE_BLANKS:
                ...
                if not zero_blank_ok:
                    diagnostics.append("Zero-blank enforcement could not eliminate all fillable blanks. Review QA tabs.")
            ...
            diagnostics.append(
                ...
                "If inspected by Python/openpyxl before Excel recalculates, formula-result columns can look blank."
            )
            ...
            st.success("✅ File-only workbook built")
            ...
            st.error("The file-only report builder hit an error. The real traceback is below.")
            st.exception(e)
        finally:
            try:
                if wb is not None:
